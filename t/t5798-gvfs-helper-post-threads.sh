...
test_expect_success 'create enough blobs for parallel POST' '
	test_commit_bulk -C "$REPO_SRC" --filename="parallel.%s.t" 101 &&
...
# Helper: POST blobs with the minimum parallel block size.
#
do_post_blobs_small_blocks () {
	git -C "$REPO_T1" gvfs-helper \
		...
		--no-progress \
		post \
		--block-size="$parallel_block_size" \
		<"$OIDS_BLOBS_FILE" >OUT.output 2>OUT.stderr &&
...
test_expect_success PTHREADS 'small block size uses sequential POST' '
	test_when_finished "per_test_cleanup" &&
	start_gvfs_protocol_server &&
	...
	git -C "$REPO_T1" gvfs-helper \
		...
		--no-progress \
		post \
		--block-size=$(($parallel_block_size - 1)) \
		<"$OIDS_BLOBS_FILE" >OUT.output 2>OUT.stderr &&
