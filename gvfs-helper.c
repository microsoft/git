...
#define GH__MIN_OBJECTS_POST__PARALLEL_BLOCK_SIZE 100
...
static void build_post_payload(struct json_writer *jw,
			       const struct object_id * const *oids,
...
{
	odb_prepare(the_repository->objects, ODB_PREPARE_FLUSH_CACHES);
	...
	const char *suffix2, struct tempfile **t2)
{
	...
	retries = 0;
	*t1 = repo_create_tempfile(repo, buf.buf);
	while (!*t1 && retries < 5) {
		retries++;
	...
	if (suffix2 && *suffix2 && t2) {
		...
		*t2 = repo_create_tempfile(repo, buf.buf);
		while (!*t2 && retries < 5) {
			retries++;
			strbuf_setlen(&buf, len_tp);
	...
	struct prefetch_entry *entry)
{
	 ...
	 */
	my_create_tempfile(status, the_repository, 0, "pack", &tempfile_pack, NULL, NULL);
	if (!tempfile_pack)
		return;
		   ...
		   struct gh__response_status *status)
{
	...
	if (params->b_write_to_file) {
		...
		my_create_tempfile(status, the_repository, 1, NULL, &params->tempfile, NULL, NULL);
...
struct post_thread_ctx {
	...
	const struct object_id * const *oid_array;
...
retry_block:
		...
		curl = td->curl;
		configure_post_curl_handle(curl, ctx, request_url,
		...
		/*
		 * A failed response may already have written part of a pack.
					...
					struct string_list *result_list)
{
	...
	if (HAVE_THREADS && gh__global.post_threads > 1 &&
	    ...
	    gh__cmd_opts.block_size >=
		    GH__MIN_OBJECTS_POST__PARALLEL_BLOCK_SIZE &&
	    nr_oid_total > 1) {
		const struct object_id *oid;
		const struct object_id **oid_array;
		int nr_workers;
		struct post_thread_ctx ctx;
		...
		/* Collect pointers to the oidset entries. */
