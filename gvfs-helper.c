 ...
 */
static struct tempfile *create_tempfile_for_packfile(struct repository *repo)
{
	...
	tempfile = repo_create_tempfile(repo, buf_path.buf);
	...
	const struct object_id *oid)
{
	...
	params->tempfile = repo_create_tempfile(repo, buf_path.buf);
	if (!params->tempfile) {
		strbuf_addstr(&status->error_message,
				   ...
				   const struct object_id *oid)
{
	...
	create_tempfile_for_loose(&params, status, the_repository, oid);
	if (!params.tempfile)
		goto cleanup;
				       ...
				       unsigned long *nr_taken)
{
	...
	*nr_taken = params.object_count;
	...
	params.tempfile = create_tempfile_for_packfile(the_repository);
	if (!params.tempfile) {
		strbuf_addstr(&status->error_message,
