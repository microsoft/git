...
static void initialize_includes_hashmap(struct hashmap *map, struct strbuf *vfs_data)
{
	 ...
	 */
	vfshash = repo_ignore_case(the_repository) ? memihash : memhash;
	vfscmp = repo_ignore_case(the_repository) ? strncasecmp : strncmp;
	...
	for (i = 0; i < virtual_filesystem_data.len; i++) {
		if (buf[i] == '\0') {
			...
			if (buf[i - 1] == '/') {
				...
				if (repo_ignore_case(the_repository)) {
					ce = index_file_exists(istate, entry, len, repo_ignore_case(the_repository));
				} else {
				...
				if (ce) {
					do {
						 ...
						 */
						if (repo_ignore_case(the_repository))
							ce = index_file_next_match(istate, ce, repo_ignore_case(the_repository));
						else
							break;
